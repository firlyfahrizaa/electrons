...
$redirect  = isset($_POST['redirect_url']) ? $_POST['redirect_url'] : 'belanja.php'; // ðŸŸ¢ Tambahkan ini
...
if ($data && $data['stok'] >= $jumlah) {
    ...
    if ($insert) {
        ...
        echo "<script>alert('Gagal menyimpan pesanan.'); window.location.href='$redirect';</script>"; // ðŸŸ¢ gunakan $redirect
    }
} else {
    echo "<script>alert('Stok tidak mencukupi.'); window.location.href='$redirect';</script>"; // ðŸŸ¢ gunakan $redirect
}
?>
